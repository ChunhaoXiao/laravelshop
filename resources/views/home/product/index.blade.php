                ...
                <section class="store-header-area panel t-xs-center t-sm-left">
                    <div class="row row-rl-10">
                        <div class="col-sm-3 col-md-2 t-center">
                            ...
                            <ol class="breadcrumb">@foreach(\App\Models\Category::ancestorsAndSelf($category) as $category) <li>{{$category->name}}</li> @endforeach</ol>
                            
                        </div>
                 <div class="col-sm-5 col-md-6">

                            <div class="store-about ptb-30">
                                        @inject('filter','App\Service\CategoryService')
                                        @foreach($filter->attributeFilter() as $k=> $attribute)

                                         <p> <ul class="list-inline"><li>{{$attribute[0]->attribute->name}}:</li>
                                            <li>
                                                @if(request()->input('attr'.$k)) 
                                                    <a href="{{ route('product.list', array_merge([$category->id],array_diff_key(request()->query(),['attr'.$k=>0]))) }}">全部</a>
                                                @else 
                                                    <span class="bg-primary">全部</span>
                                                @endif
                                             </li> 
                                             @foreach($attribute as $value)
                                                  <li>@if($value->querystring) 
                                                    <a href="{{route('product.list', $value->querystring)}}">{{$value->attribute_value}}</a>
                                                    @else
                                                   <span class="bg-primary"> {{$value->attribute_value}}</span>
                                                    @endif
                                                </li>
                    ...
                    <header class="page-control panel ptb-15 prl-20 pos-r mb-30">
                        ...
                        <div class="pos-tb-center">
                           <ul class="list-inline">
                            ...
                            @else
                                 <li><a  href="{{route('product.list',array_merge([$category->id],request()->query(),['order'=>'created_at']))}}">最新</a></li>
                            @endif    

                            @if(in_array(request()->order,['price_desc','price_asc']))
                                @if(request()->order == 'price_asc')
                                    <li class="bg-info"><a href="{{ route('product.list',array_merge([$category->id],request()->query(),['order'=>'price_desc'])) }}">价格</a></li> 
                                @else
                                    <li class="bg-info"><a href="{{ route('product.list',array_merge([$category->id],request()->query(),['order'=>'price_asc'])) }}">价格</a></li> 
                                @endif    
                            @else
                                <li><a href="{{ route('product.list',array_merge([$category->id], request()->query(),['order'=>'price_asc'])) }}">价格</a></li>
                            @endif
                            @if(request()->order == 'popular')
                                <li class="bg-info">最受欢迎</li>
                            @else
                                <li><a href="{{ route('product.list',array_merge([$category->id],request()->query(),['order' => 'popular'])) }}">最受欢迎</a></li>
                            @endif
        ...
        $(function(){
            $("#order-select").on('change',function(){
                ...
                @php 
                    $query = request()->query()? :[] ;
                    unset($query['order']);
                    $url = route('product.list',array_merge([$category->id], $query));
                @endphp
                var url = "{{$url}}";
